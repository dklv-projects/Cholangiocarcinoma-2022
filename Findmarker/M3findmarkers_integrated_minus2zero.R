...
#Rscript ../script/M3findmarkers.R ../M2classification/M2B_cells.RDS integrated_snn_res.0.5
#Rscript /home/dklv/scPip/script/M3findmarkers.R M1scCancer/comb/expr_combined.RDS integrated_snn_res.0.2
...
pwd=getwd()
setwd(workdir)
savePath=paste0(pwd,"/M3markers_",gsub("\\..*","",exprobj))
...
DefaultAssay(expr_comb)="integrated"
rewrite=FALSE
...
library(Matrix)
mtx=as.matrix(expr_comb@assays$integrated@data)
mtx[mtx<0]=0
expr_comb@assays$integrated@data=as(mtx, "dgCMatrix")
...
cluster_mean_expression2=data.frame()
cluster_percent_expression2=data.frame()
...
matexpr2=as.matrix(expr_comb@assays$RNA@data)[rownames(matexpr),colnames(matexpr)]
for(clust in sort(unique(meta$seurat_clusters))){
  ...
  exprmat2=matexpr2[,cells]
  otherexprmat=matexpr[,othercells]
  otherexprmat2=matexpr2[,othercells]  
  rowmean=rowMeans(exprmat)
  rowmean2=rowMeans(exprmat2)
  pct.cells=apply(exprmat2,1,function(x){sum(x>0)/length(x)})
  pct.othercells=apply(otherexprmat2,1,function(x){sum(x>0)/length(x)})
  pct.df2=data.frame(value1=pct.cells,value2=pct.othercells)
  colnames(pct.df2)=c(clust,paste0("not_",clust))  
  if(ncol(cluster_percent_expression2)==0){
    cluster_percent_expression2=pct.df2
  }else{
    cluster_percent_expression2=cbind(cluster_percent_expression2,pct.df2)    
  }
  
  otherrowmean=rowMeans(otherexprmat)
  otherrowmean2=rowMeans(otherexprmat2)
  ...
  rowmeandf2=data.frame(value1=rowmean2,value2=otherrowmean2)
  colnames(rowmeandf)=c(clust,paste0("not_",clust))
  colnames(rowmeandf2)=c(clust,paste0("not_",clust))  
  ...
  if(ncol(cluster_mean_expression2)==0){
    cluster_mean_expression2=rowmeandf2
  }else{
    cluster_mean_expression2=cbind(cluster_mean_expression2,rowmeandf2)    
  }
...
write.csv(cluster_mean_expression2,file = file.path(savePath,"M3_cluster_mean_expression_RNA.csv"))
write.csv(cluster_percent_expression2,file = file.path(savePath,"M3_cluster_percent_expression_RNA.csv"))



correct.percent.RNA=function(diff.expr.genes,cluster_percent_expression){
  diff.expr.genes.new=data.frame()
  for(clust in sort(unique(diff.expr.genes$cluster))){
    col.name=c(clust,paste0("not_",clust))
    dg.clust=diff.expr.genes[diff.expr.genes$cluster==clust,]
    percent.correct=cluster_percent_expression[dg.clust$gene,col.name]
    colnames(percent.correct)=c("pct.1.RNA","pct.2.RNA")
    diff.expr.genes.new=rbind(diff.expr.genes.new,cbind(dg.clust,percent.correct))
  }
  diff.expr.genes.new=diff.expr.genes.new[diff.expr.genes.new$pct.1.RNA>0.1,]
  return(diff.expr.genes.new)
}
...
diff.expr.genes <- Seurat::FindAllMarkers(expr_comb,only.pos = T,logfc.threshold = 0.25) 
...
diff.expr.genes = correct.percent.RNA(diff.expr.genes,cluster_percent_expression2)
...
#筛选方法1B，严谨
diff.expr.genes2 <- Seurat::FindAllMarkers(expr_comb,only.pos = T,logfc.threshold = 0.25,test.use = "roc")
...
diff.expr.genes2 = correct.percent.RNA(diff.expr.genes2,cluster_percent_expression2)
